...
RUNNING_THRESHOLD = 0.3 # Time in seconds between left knee and right knee reaching MOVEMENT_THRESHOLD to constitute as "RUNNING"
...
def yoloDisplay():
    global frame, leftAngle, rightAngle, lateralState, leftHip, leftKnee
    cv2.putText(frame, "{:.2f}".format(leftAngle), (0,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    cv2.putText(frame, "{:.2f}".format(rightAngle), (0,75), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    cv2.putText(frame, lateralState, (0,100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    # cv2.putText(frame, "Left Hip: {:.2f}".format(leftHip[0]), (0,300), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    # cv2.putText(frame, "Left Knee: {:.2f}".format(leftKnee[0]), (0,325), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    cv2.putText(frame, "NZD Hip: {:.2f}".format(normalizedHip), (0,325), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
...
def yoloPredict():
    global frame, state, leftHip, leftKnee, leftAngle, rightAngle, normalizedQuad, normalizedHip
    while True:
        ...
        try:
            ...
            # length of hip
            h1 = math.pow(rightHip[0]-leftHip[0], 2) + math.pow(rightHip[1]-leftHip[1], 2)
...
def movementState():
    global lateralState, verticalState, leftHip, leftKnee, strafeDirection, normalizedQuad, normalizedHip, leftHip, rightHip
    leftLegTime = 0
    rightLegTime = 0
    timeOfActivity = 0
    lastStrafeTime = 0
    while True:
        ...
        # denote the time at which legs bent
        if leftAngle < RUNNING_ANGLE:
            leftLegTime = time.time() 
        if rightAngle < RUNNING_ANGLE:
            rightLegTime = time.time()

        # if legs bent a certain time between each other, then constitute as "RUNNING"
        ...
        if RUNNING_THRESHOLD_MIN < runningTime < RUNNING_THRESHOLD:
            ...
            leftLegTime = timeOfActivity
            rightLegTime = timeOfActivity

            if normalizedQuad > 1:
                strafeDirection = -20000 # strafe left
                
            elif normalizedQuad < -1:
                if normalizedHip < 0.1:
                    strafeDirection = 20000 # strafe right
