...
void loop() {
    ...
    output[1] = (analogRead(joyXPin) >> 4); // right shift 4-bits since SerialBT.write only sends uint8_t and ADC 2^12
    output[2] = (analogRead(joyYPin) >> 4); // right shift 4-bits since SerialBT.write only sends uint8_t and ADC 2^12
    output[3] = digitalRead(joySWPin); // read if joystick button is pressed
